...
<main class="main-container">
    ...
    <div class="section-box">
        ...
        <p>
            <textarea id="vak1_werkbeschrijving" class="editable-field" rows="4" style="width: 100%;"></textarea>
        </p>
    ...
    <div class="section-box">
        ...
        <p>
            <textarea id="vak3_aandachtspunten" class="editable-field" rows="4" style="width: 100%;"></textarea>
        </p>
    ...
    <div class="section-box">
        ...
        <p>
            <textarea id="vak4_aandachtspunten" class="editable-field" rows="4" style="width: 100%;"></textarea>
        </p>
    </div>
    ...
    <div class="section-box">
        ...
        <img id="vak7_inspecteur_handtekening" class="signature-img" alt="">
    </div>
    ...
    <div class="navigation-buttons" style="margin-top: 30px; padding: 20px 0;">
        ...
        <form id="aanvraagOpslaanForm" action="../pages/aanvraag_opslaan.php" method="POST" style="display:inline;">
            <input type="hidden" name="aanvraag_data" id="aanvraag_data">

            <button class="nav-button button next" type="submit">
                Werkvergunning opslaan &amp; terug naar overzicht
            </button>
        </form>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function fillCheckboxList(id, key) {
        ...
        el.innerHTML = data
            .split(',')
            .map(v => `<span class="checkbox-item">${escapeHtml(v.trim())}</span>`)
            .join('');
    }

    function safeJsonArray(storageKey) {
        try {
    ...
    function fillTable(tableId, storageKey, rowBuilder) {
        const data = safeJsonArray(storageKey);
    ...
    function saveAllChanges() {
        ...
        fieldIds.forEach(id => saveText(id));
    }
    ...
    function collectFieldValues() {
        ...
        fieldIds.forEach(id => {
            const el = document.getElementById(id);
            fields[id] = el && (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') ? el.value : '';
        });

        return fields;
    }

    function collectAanvraagData() {
        saveAllChanges();
        ...
        return {
            ...
            tables: {
                vak6_tabel: sessionStorage.getItem('vak6_tabel') || '[]',
                vak7_tabel: sessionStorage.getItem('vak7_tabel') || '[]'
            },
            signatures: {
                inspecteur: sessionStorage.getItem('handtekening_inspecteur') || ''
            }
        };
    }
    ...
    /* ---------- Vak III ---------- */
    fillText('vak3_aandachtspunten');
    fillText('vak3_parkeerplaats');

    /* ---------- Vak IV ---------- */
    fillText('vak4_naam');
    fillText('vak4_afdeling');
    fillText('vak4_aandachtspunten');
    ...
    /* ---------- Vak VI ---------- */
    fillText('vak6_lototo');
    fillText('vak6_afdeling');
    fillText('vak6_uitvoerder');

    fillTable('vak6_tabel', 'vak6_tabel', row => `
        <td>${escapeHtml(row.dag || '')}</td>
        <td>${escapeHtml(row.datum || '')}</td>
        <td>${escapeHtml(row.van || '')}</td>
        <td>${escapeHtml(row.tot || '')}</td>
        <td>${escapeHtml(row.afdelingNaam || '')}</td>
        <td>${escapeHtml(row.afdelingParaaf || '')}</td>
        <td>${escapeHtml(row.uitvoerderNaam || '')}</td>
        <td>${escapeHtml(row.uitvoerderParaaf || '')}</td>
        <td>${escapeHtml(row.aantalUitvoerders || '')}</td>
        <td>${escapeHtml(row.afdOverdracht || '')}</td>
    `);

    /* ---------- Vak VII ---------- */
    fillText('vak7_inspectie');
    ...
    fillTable('vak7_tabel', 'vak7_tabel', row => `
        ...
        <td>${row.handtekening ? `<img src="${escapeHtml(row.handtekening)}" class="signature-img" alt="">` : ''}</td>
    `);

    const sig = sessionStorage.getItem('handtekening_inspecteur');
    const sigEl = document.getElementById('vak7_inspecteur_handtekening');

    if (sig && sigEl) {
        sigEl.src = sig;
    }

    const aanvraagOpslaanForm = document.getElementById('aanvraagOpslaanForm');

    if (aanvraagOpslaanForm) {
        aanvraagOpslaanForm.addEventListener('submit', function () {
            const data = collectAanvraagData();
            document.getElementById('aanvraag_data').value = JSON.stringify(data);
        });
    }
});
</script>
